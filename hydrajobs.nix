...
{self, ...}: let
  ...
  mkGpioEnabled = cfg: let
    gpioEnableModule = {lib, ...}: {
      ghaf.hardware.nvidia = {
        virtualization.enable = lib.mkForce true;
        virtualization.host.gpio.enable = lib.mkForce true;
      };
    };
    newCfg = cfg.extendModules {modules = [gpioEnableModule];};
    package = newCfg.config.system.build.${newCfg.config.formatAttr};
  in
  package;
...
in {
  flake.hydraJobs = {
    ...
    # GPIO virt enabled versions
    nvidia-jetson-orin-agx-debug-gpio.aarch64-linux = mkGpioEnabled self.nixosConfigurations.nvidia-jetson-orin-agx-debug;
    nvidia-jetson-orin-nx-debug-gpio.aarch64-linux = mkGpioEnabled self.nixosConfigurations.nvidia-jetson-orin-nx-debug;
    nvidia-jetson-orin-agx-debug-gpio-from-x86_64.x86_64-linux = mkGpioEnabled self.nixosConfigurations.nvidia-jetson-orin-agx-debug-from-x86_64;
    nvidia-jetson-orin-nx-debug-gpio-from-x86_64.x86_64-linux = mkGpioEnabled self.nixosConfigurations.nvidia-jetson-orin-nx-debug-from-x86_64;
  };
}
